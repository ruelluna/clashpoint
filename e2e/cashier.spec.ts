...
async function enableEventFees(page: Page) {
  ...
  const cashBondSection = page

    .locator('div')

    .filter({ has: page.getByText('Cash bond (per entry)', { exact: true }) })

    .first()

  const cashBondSwitch = cashBondSection.getByRole('switch')

  if (await cashBondSwitch.count()) {

    const checked = await cashBondSwitch.getAttribute('data-checked')

    if (checked == null) {

      await cashBondSwitch.click()

    }

    await page.locator('input[name="cashBondAmount"]').fill('1000')
...
test.describe('Cashier Terminal @auth', () => {
  ...
  test('opens session, collects registration dues, and offers itemized print', async ({ page }) => {
    ...
    if (await amountInput.count()) {
      ...
      await expect(
        ...
        page.getByText(/Collects registration, rooster entry, and cash bond/i)
      ...
      const printLink = page.getByRole('link', { name: 'Print receipt' })
      ...
      await expect(printLink).toHaveAttribute(
        ...
        new RegExp(`/dashboard/events/${eventId}/payments/batch/[^/]+/print`)
