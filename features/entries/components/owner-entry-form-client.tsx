...
type OwnerEntryFormClientProps = {
  eventId: string
  eventName: string
...
export function OwnerEntryFormClient({
  eventId,
  eventName,
  promoters,
}: OwnerEntryFormClientProps) {
  ...
  return (
    <PageStack maxW="2xl">
      <PageHeader
        title="Register owner / game farm"
        description={`${eventName} · Owner registration only. Add roosters on the Roosters tab after this step.`}
      />
        ...
        <Stack gap={LAYOUT_GAP.form}>
          ...
          <ButtonGroup mt={LAYOUT_GAP.form}>
            ...
            <Button asChild variant="outline">
              <Link href={`/dashboard/events/${eventId}/owners`}>Cancel</Link>
            </Button>
          </ButtonGroup>
        </Stack>
      </form>
