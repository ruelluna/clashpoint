...
import type { PublicStanding } from '@/features/public/types'
  ...
  return (
    <Box borderWidth="1px" borderColor="border" rounded="lg" overflow="hidden">
      <Box p={4} borderBottomWidth="1px" borderColor="border">
        <Text fontWeight="medium">Standings</Text>
        <Text fontSize="sm" color="fg.muted">
          Ranked by points, then wins.
        </Text>
      </Box>
      {standings.length === 0 ? (
        <Box p={4}>
          <Text fontSize="sm" color="fg.muted">
            Standings will appear once results are verified.
          </Text>
        </Box>
      ) : (
        <Box overflowX="auto">
          <Box as="table" width="full" fontSize="sm">
            <Box as="thead" bg="bg.subtle">
              <Box as="tr">
                {['Rank', 'Entry', 'Owner', 'W', 'L', 'D', 'Points'].map((heading) => (
                  <Box
                    as="th"
                    key={heading}
                    textAlign="left"
