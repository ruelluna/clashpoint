...
function vipSideFromDescription(description: string | null): ResultVipSettlementItem['side'] {
  if (!description) return null
  const side = description.split(' · ')[0]?.trim()
  return side === 'Meron' || side === 'Wala' ? side : null
}
...
function mapVipSettlement(row: ObligationRow): ResultVipSettlementItem {
  ...
  return {
    ...
    side: vipSideFromDescription(row.description),
...
export async function listResultsForEvent(
    ...
    const ledgerObligations = matchObligations.filter(
      (obligation) =>
    ...
    const revolvingFundComplete =
      ledgerObligations.length === 0 ||
      ledgerObligations.every((obligation) => obligation.status === 'posted')
        ...
        return {
            ...
            matching_number: row.matches?.matching_number ?? null,
